...
import { MedicalAndDentalPageMsg } from '../../../pages/MediAndDental/medicalAndDentalPage_msg';
...
test('Family with 4 children Medi Dent test', async ({ page }) => {
  const  mediDentPage = new MedicalAndDentalPageMsg(page);
...
await expect(page).toHaveURL("https://individualinsurance-stg.healthpartners.com/hp/shopping/anonymous.html#view/account/WhosCoveredSE/Demographics");
    ...
    await mediDentPage.selectDependentGender(fakeValue.childGenderLocator);
    await mediDentPage.selectSmokerOptionForDepandant(fakeValue.childSmokerLocator);
