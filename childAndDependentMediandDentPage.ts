...
export class ChildDependentMedicalAndDentalPage {
  ...
  readonly p_depFirstName: Locator;
  readonly p_depLastName: Locator;
  readonly p_depBirthDate: Locator;
  readonly p_childrelationshipLocator: Locator;
  readonly p_childGenderLocator: Locator;
  ...
  constructor(page: Page) {
    ...
    this.zip4Locator = this.page.locator('input[name="Application\\.Applicant\\.Zip_Plus_Four"]'); 
    ...
    this.p_depFirstName = this.page.locator('#firstName44');
    this.p_depLastName = this.page.locator('#lastName45');
    this.p_depBirthDate = this.page.locator('#birthDate46');
    this.p_childrelationshipLocator = this.page.locator('#memberRelationship47');
    this.p_childGenderLocator = this.page.locator('#gender48');
...
async fillDepFirstName(firstName: string){
    await this.p_depFirstName.click();
    await this.p_depFirstName.fill(firstName); 
}

async fillDepLastName(lastName: string){
    await this.p_depLastName.click();
    await this.p_depLastName.fill(lastName);
}

async fillDepBirthDate(birthDate: string){
    await this.p_depBirthDate.click();
    await this.p_depBirthDate.fill(birthDate);
}

async selectDependentRelation(relationship: string) {
    await this.p_childrelationshipLocator.selectOption(relationship);
  }

async selectDepGender(gender: string) {
    await this.p_childGenderLocator.selectOption(gender);
  }
...
async applicationInfo(address: string, city: string, phone: string, SSN: string){
  ...
  await this.addressLocator.click();
  await this.addressLocator.fill('718 Jefferson St NE');
  await this.cityLocator.click();
  await this.cityLocator.fill('Minneapolis');
  await this.zip4Locator.fill('2111');
  await this.phoneLocator.click();
  await this.phoneLocator.fill(phone);
  // await this.page.getByRole('textbox', { name: '###-##-####' }).click()
  // await this.page.getByRole('textbox', { name: '###-##-####' }).fill('123451111');
...
async currentPreviousDentIns(){
  ...
  await this.page.getByLabel('No').nth(2).check();//no current or previous dent covg w/in last 3 months
  await this.page.getByRole('link', { name: 'Continue ' }).click();
}

async currentPreviousDentCvg(){
  await this.page.getByLabel('No').first().check(); 
  // await this.page.getByLabel('No').nth(2).check();//no current or previous dent covg w/in last 3 months
  await this.page.getByRole('link', { name: 'Continue ' }).click();
}
